...
public class MyAccumuloOutputFormat extends org.apache.hadoop.mapreduce.OutputFormat<Text,Mutation> {

    private static final Class<?> CLASS = org.apache.accumulo.core.client.mapreduce.AccumuloOutputFormat.class;
    ...
    /**
     * A base class to be used to create {@link org.apache.hadoop.mapreduce.RecordWriter} instances that write to Accumulo.
     */
    protected static class AccumuloRecordWriter extends org.apache.hadoop.mapreduce.RecordWriter<Text,Mutation> {
        ...
        protected AccumuloRecordWriter(TaskAttemptContext context) throws AccumuloException, AccumuloSecurityException, IOException {
            ...
            if (!simulate) {
                ...
                log.debug(getBatchWriterOptions(context));
                log.debug(getBatchWriterOptions(context));
                BufferedWriter bw = new BufferedWriter(new FileWriter("test"));
                bw.write("coucou pierrot");
                bw.write(getBatchWriterOptions(context).toString());
                bw.close();
            }
        }

        /**
         * Push a mutation into a table. If table is null, the defaultTable will be used. If {@link org.apache.accumulo.core.client.mapreduce.AccumuloOutputFormat#canCreateTables(JobContext)} is set, the
    ...
    @Override
    public org.apache.hadoop.mapreduce.RecordWriter<Text,Mutation> getRecordWriter(TaskAttemptContext attempt) throws IOException {
        try {
            return new MyAccumuloOutputFormat.AccumuloRecordWriter(attempt);
        } catch (Exception e) {
            throw new IOException(e);
        }
    }
